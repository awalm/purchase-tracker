...
struct BackupPurchaseAllocation {
    ...
    legacy_receipt_line_item_id: Option<Uuid>,
    allocated_qty: i32,
    unit_cost: Decimal,
}
...
async fn export_backup(
    ...
    let invoice_document_path = queries::get_invoice_pdf(&state.pool, id)
        .await
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))?
        .map(|(bytes, filename)| {
    ...
    for receipt_id in sorted_receipt_ids {
        ...
        let receipt_document_path = queries::get_receipt_pdf(&state.pool, receipt_id)
            .await
            .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))?
            .map(|(bytes, filename)| {
        ...
        backup_receipts.push(BackupReceipt {
            ...
            payment_card_last4: receipt.payment_card_last4,
    ...
    let mut backup_purchases: Vec<BackupPurchase> = purchases
        ...
        .map(|purchase| {
            ...
            BackupPurchase {
                legacy_id: purchase.purchase_id,
                ...
                legacy_receipt_id: purchase.receipt_id,
                allocations: allocations
                    .into_iter()
                    .map(|allocation| BackupPurchaseAllocation {
                        legacy_receipt_id: allocation.receipt_id,
                        legacy_receipt_line_item_id: allocation.receipt_line_item_id,
    ...
    backup_purchases.sort_by_key(|row| row.legacy_id);
    backup_receipts.sort_by_key(|row| row.legacy_id);
...
async fn import_backup(
    ...
    let mut zip_payload: Option<Vec<u8>> = None;
        ...
        .map_err(|e| (StatusCode::BAD_REQUEST, e.to_string()))?
    {
        ...
        if name == "file" {
            let bytes = field
                ...
                .await
                .map_err(|e| (StatusCode::BAD_REQUEST, e.to_string()))?;
            zip_payload = Some(bytes.to_vec());
            break;
        }
    }

    let zip_payload = zip_payload.ok_or((
    ...
    for receipt in &bundle.receipts {
        ...
        let restored_receipt = loop {
            ...
            let create_result = queries::create_receipt(
                ...
                CreateReceipt {
                    ...
                    payment_card_last4: receipt.payment_card_last4.clone(),
        ...
        receipt_id_map.insert(receipt.legacy_id, restored_receipt.id);
        ...
        for line_item in &receipt.line_items {
            ...
            receipt_line_item_id_map.insert(line_item.legacy_id, created_line_item.id);
        }
    }
    ...
    for purchase in &bundle.purchases {
        ...
        let direct_receipt_id = if purchase.allocations.is_empty() {
            purchase
                .legacy_receipt_id
                .and_then(|legacy_receipt_id| receipt_id_map.get(&legacy_receipt_id).copied())
        } else {
            None
        };
        ...
        purchase_id_map.insert(purchase.legacy_id, created_purchase.id);
    }

    let mut restored_allocation_count = 0usize;
    for purchase in &bundle.purchases {
        let restored_purchase_id = purchase_id_map.get(&purchase.legacy_id).copied().ok_or((
            StatusCode::UNPROCESSABLE_ENTITY,
            format!(
        ...
        for allocation in &purchase.allocations {
            let legacy_line_item_id = allocation.legacy_receipt_line_item_id.ok_or((
                StatusCode::UNPROCESSABLE_ENTITY,
                format!(
    ...
    Ok((
        StatusCode::CREATED,
        Json(InvoiceBackupImportResponse {
